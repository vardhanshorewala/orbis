...
const pascal_case_1 = require("../utils/change-case/pascal-case");
    ...
    w.inIndent(() => {
        if (abi.errors) {
            Object.entries(abi.errors).forEach(([k, abiError]) => {
                w.append(`${k}: { message: ${JSON.stringify(abiError.message)} },`);
            });
        }
    });
    w.append(`} as const`);
    ...
    w.inIndent(() => {
        if (abi.getters) {
            for (const t of abi.getters) {
                w.append(JSON.stringify(t) + ",");
                let getterName = (0, pascal_case_1.pascalCase)(t.name);

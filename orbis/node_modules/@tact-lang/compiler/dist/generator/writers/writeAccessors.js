    ...
    ctx.fun(ops_1.ops.typeFromTuple(type.name, ctx), () => {
        ...
        ctx.body(() => {
            ...
            if (vars.length <= writeStruct_1.maxTupleSize) {
                ...
                const firstBatch = vars.splice(0, writeStruct_1.maxTupleSize - 1);
                ctx.used(`__tact_tuple_destroy_${writeStruct_1.maxTupleSize}`);
                ctx.append(`var (${firstBatch.join(", ")}, next) = __tact_tuple_destroy_${writeStruct_1.maxTupleSize}(v);`);
                while (vars.length >= writeStruct_1.maxTupleSize) {
                    const nextBatch = vars.splice(0, writeStruct_1.maxTupleSize - 1);
                    ctx.used(`__tact_tuple_destroy_${writeStruct_1.maxTupleSize}`);
                    ctx.append(`var (${nextBatch.join(", ")}, next) = __tact_tuple_destroy_${writeStruct_1.maxTupleSize}(next);`);
                }
                ctx.used(`__tact_tuple_destroy_${vars.length}`);
                ctx.append(`var (${vars.join(", ")}) = __tact_tuple_destroy_${vars.length}(next);`);
            }
